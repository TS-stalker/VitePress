## 1. Overview

Straight line shape controls are used to create and edit straight line graphic elements on the canvas. As basic connection and separation tools, they play an important role in interface layout, chart creation, and visual guidance.

![Image Description](../../images/控件照片/直线/1.png)

## 2. Usage Scenarios

Straight line controls are used in the following scenarios:

### 1. Data Visualization Scenarios

- Used as auxiliary shapes for graphical display of data relationships
- Straight line control elements in data distribution diagrams and relationship diagrams
- Straight line control backgrounds for progress indicators and status identifiers

## 3. Quick Start

1. Select the straight line control from the toolbar
2. Place the mouse at the starting position on the canvas, hold down the left mouse button
3. Drag the mouse to the end position to determine the line direction and length
4. Release the left mouse button to complete straight line creation
5. Adjust line length and direction through the endpoint control points of the selection box
6. Use the rotation control point or middle control point to adjust angle and position
7. Precisely set length, angle, and style parameters in the property panel

**Example:**

### 1.1 Data Visualization Scenario Example

This example is creating a round rectangle and setting basic properties, as shown in Figure 1-1.

![Image Description](../../images/控件照片/直线/2.gif)

Figure 1-1

1.2 Actual Configuration Tutorial:

1. Drag the straight line control to the canvas and adjust position and size
2. You can set the line style and thickness of the straight line, then save and run

## 4. Property Details

| Name   | Description                                                   |
| ------ | ------------------------------------------------------------- |
| Name   | The name of this control.                                         |
| X      | The distance from the left side of the control to the left side of the canvas, in px.                     |
| Y      | The distance from the top of the control to the top of the canvas, in px.                     |
| W      | The width of the control, in px.                                   |
| H      | The height of the control, in px.                                   |
| Angle   | The rotation angle of the control                                         |
| Opacity | The opacity of the control                                           |
| Line   | The line color of the control<br />The line style of the control<br />The line thickness of the control |
| Permission   | Set the permissions of the control, control display/hide or enable/disable                  |
